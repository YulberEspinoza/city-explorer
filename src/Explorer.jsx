...
function Explorer() {
  ...
  const displayWeather = async (lat, lon) => {
    try {
      ...
      displayMovie(weather.data.country_code);
    } catch (error) {
      setDisplayMap(false);
      setDisplayError(true);
      if (error.response) {
  ...
  const displayMovie = async (country) => {
    try {
      let movie = await axios.get(`${SERVER_URL}movie`, {
    ...
    <Container>
      ...
      {displayMap && (
        <>
          <Row>
            <Col>
              <LatLon city={location} lat={latitude} lon={longitude} />
            </Col>
          </Row>
          <Row>
            <Col>
              <Map
                img_url={`https://maps.locationiq.com/v3/staticmap?key=${API_KEY}&center=${latitude},${longitude}&zoom=16&size=600x400&format=jpg&maptype=streets`}
                city={location}
              />
            </Col>
          </Row>
